package com.mordecai.base.ui.component.blurlayout;
...
public class BlurLayout extends FrameLayout {
    ...
    private String mText = "正在加载中...";
    private ColorStateList mFill = getColorStateListFromAttr(getContext(), com.google.android.material.R.attr.colorPrimaryContainer).withAlpha(140);
    private ColorStateList mStroke = getColorStateListFromAttr(getContext(), com.google.android.material.R.attr.colorPrimary).withAlpha(140);

    public static ColorStateList getColorStateListFromAttr(Context context, int attribute) {
        TypedValue typedValue = new TypedValue();
        context.getTheme().resolveAttribute(attribute, typedValue, true);
        TypedArray typedArray = context.obtainStyledAttributes(typedValue.data, new int[]{attribute});
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        try {
            ...
            float x = getMeasuredWidth() / 2f - textWidth / 2f;
            float y = getMeasuredHeight() - getTextHeight(mPaint.getFontMetrics()) - getPaddingBottom();
            mPaint.setStyle(Paint.Style.STROKE);
            mPaint.setColor(mStroke.getDefaultColor());
            canvas.drawText(mText, x, y, mPaint);
            mPaint.setStyle(Paint.Style.FILL);
            mPaint.setColor(mFill.getDefaultColor());
            canvas.drawText(mText, x, y, mPaint);
        } finally {
    ...
    private void init(Context context) {
        mPaint.setTextSize(getResources().getDimension(R.dimen.base_sp12));
        mPaint.setStrokeWidth(2f * getContext().getResources().getDisplayMetrics().density);
