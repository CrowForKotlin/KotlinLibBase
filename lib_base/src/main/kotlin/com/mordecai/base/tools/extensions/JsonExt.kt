...
inline fun<reified T> toTypeEntity(value: Any?, vararg clazz: Class<*>, moshi: Moshi = baseMoshi): T? {
    if (value != null) {
        val type = Types.newParameterizedType(clazz.first(), *clazz.drop(1).toTypedArray())
        return if (value is String) moshi.adapter<T>(type).fromJson(value) else moshi.adapter<T>(type).fromJsonValue(value)
    } else {
        return null
    }
}
