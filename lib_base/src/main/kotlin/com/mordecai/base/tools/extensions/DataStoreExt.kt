...
val Context.appConfigDataStore: DataStore<Preferences> by preferencesDataStore(app.getString( R.string.base_app_name).plus(DataStoreAgent.APP_CONFIG.name))
val Context.appBookDataStore: DataStore<Preferences> by preferencesDataStore(app.getString(R.string.base_app_name).plus(DataStoreAgent.DATA_BOOK.name))
...
suspend fun<T> Preferences.Key<T>.remove(): Preferences {
    return runBlocking { app.appDataStore.edit { it.remove(this@remove) } }
}
